...
SOURCE_REPO="${CORTANA_SOURCE_REPO:-$(cd "$SCRIPT_DIR/../.." && pwd)}"
EXTERNAL_REPO="${CORTANA_EXTERNAL_REPO:-$HOME/Developer/cortana-external}"
...
normalize_task_type() {
  local raw="$1"
  local t
  t="$(printf '%s' "$raw" | tr '[:upper:]' '[:lower:]' | tr '-' '_' | xargs)"
  case "$t" in
...
TASK_TYPE_RAW="$(echo "$TASK_ROW" | jq -r '.metadata.task_type // "unknown"')"
TASK_TYPE="$(normalize_task_type "$TASK_TYPE_RAW")"
...
if [[ -z "$TASK_TYPE" ]]; then
  REASON="Skipped non-fatal: missing/unknown task_type (raw='${TASK_TYPE_RAW}')"
  esc_reason="$(sql_escape "$REASON")"
  psql "$DB" -c "UPDATE cortana_tasks SET status='ready', outcome='${esc_reason}', metadata=COALESCE(metadata,'{}'::jsonb)||jsonb_build_object('normalized_task_type',NULL) WHERE id=${TASK_ID};" >/dev/null
  audit_event "auto_executor_task_type_unknown" "warning" "$REASON" "{\"task_id\":${TASK_ID},\"task_type_raw\":\"${TASK_TYPE_RAW}\",\"allowlist\":\"${ALLOW_TASK_TYPES}\"}"
  echo "$REASON"
  exit 0
fi

if ! is_type_allowed "$TASK_TYPE"; then
  REASON="Skipped non-fatal by task type allowlist: task_type='${TASK_TYPE}'"
  esc_reason="$(sql_escape "$REASON")"
  psql "$DB" -c "UPDATE cortana_tasks SET status='ready', outcome='${esc_reason}', metadata=COALESCE(metadata,'{}'::jsonb)||jsonb_build_object('normalized_task_type','${TASK_TYPE}') WHERE id=${TASK_ID};" >/dev/null
  audit_event "auto_executor_task_type_blocked" "warning" "$REASON" "{\"task_id\":${TASK_ID},\"task_type\":\"${TASK_TYPE}\",\"task_type_raw\":\"${TASK_TYPE_RAW}\",\"allowlist\":\"${ALLOW_TASK_TYPES}\"}"
  echo "$REASON"
  exit 0
fi

GOVERNOR_JSON="$(python3 "$SOURCE_REPO/tools/governor/risk_score.py" --db "$DB" --task-json "$TASK_ROW" --actor "$ASSIGNED" --log --apply-task-state)"
...
RUN_ID_FROM_OUTPUT="$(extract_run_id "$OUT")"
ESC_RUN_ID="$(sql_escape "$RUN_ID_FROM_OUTPUT")"
...
if [[ $RC -eq 0 ]]; then
  ...
  log_idempotency "$OPERATION_ID" "$OPERATION_TYPE" "completed" "$(jq -cn --argjson task_id "$TASK_ID" --arg title "$TITLE" --arg run_id "$RUN_ID_FROM_OUTPUT" '{task_id:$task_id,title:$title,run_id:($run_id|if length>0 then . else null end),rc:0}')"
  ...
  log_idempotency "$OPERATION_ID" "$OPERATION_TYPE" "failed" "$(jq -cn --argjson task_id "$TASK_ID" --arg title "$TITLE" --arg run_id "$RUN_ID_FROM_OUTPUT" --argjson rc "$RC" '{task_id:$task_id,title:$title,run_id:($run_id|if length>0 then . else null end),rc:$rc}')"
