...
- `--auto-fix`
  ...
  - Appends an **orphan docs snapshot** section to `docs/system-hygiene-sweep.md` listing currently-unreferenced docs
...
2. **System hygiene doc**
   - `docs/system-hygiene-sweep.md` is created if missing with a basic header
...
3. **Git commit policy**
   - If `git status --porcelain` was empty at the start of the run (clean tree):
     - `TOOLS.md` and `docs/system-hygiene-sweep.md` are staged
