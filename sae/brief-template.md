...
| followups | open | cortana_human_required_actions table |
...
## Morning Brief Operational Follow-up Block (live)

Add this block to morning brief generation so unresolved operator action is always visible:

```bash
FOLLOWUPS=$(psql cortana -t -A -c "SELECT json_agg(t) FROM (SELECT id, title, system, severity, due_at FROM cortana_human_required_actions WHERE status='open' ORDER BY CASE severity WHEN 'critical' THEN 0 WHEN 'high' THEN 1 WHEN 'medium' THEN 2 ELSE 3 END, due_at ASC NULLS LAST, last_seen_at DESC LIMIT 7) t;")
```

If the follow-up result set is empty, output: `🧾 No open human-required follow-ups`.
