...
npx tsx tools/memory/extract_facts.ts extract --input <file_or_-> [--dry-run]

# Batch process recent OpenClaw session logs
npx tsx tools/memory/extract_facts.ts batch --since-hours 24 [--dry-run]
```
...
4. Embed each fact using local embeddings (`tools/embeddings/embed.ts`)
...
`extract_facts.ts` ensures schema columns exist before processing:
...
npx tsx tools/memory/extract_facts.ts extract --input /tmp/fact-sample.txt --dry-run
