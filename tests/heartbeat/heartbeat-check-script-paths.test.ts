...
describe("heartbeat check script compatibility paths", () => {
  ...
  it("provides system hygiene entrypoint", () => {
    expect(existsSync("tools/hygiene/sweep.ts")).toBe(true);
    const src = readFileSync("tools/hygiene/sweep.ts", "utf8");
    expect(src).toContain("subagent_sessions");
  });

  it("provides GitHub issue reporter entrypoint", () => {
    expect(existsSync("tools/github/issue-reporter.ts")).toBe(true);
    const src = readFileSync("tools/github/issue-reporter.ts", "utf8");
