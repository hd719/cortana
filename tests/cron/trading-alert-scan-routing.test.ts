...
describe("CANSLIM trading alert cron routing", () => {
  it("uses the unified trading runner wrapper and Monitor delivery ownership", () => {
    ...
    expect(fs.existsSync("tools/trading/run-trading-cron-alert.sh")).toBe(true);

    const message = String(job?.payload?.message ?? "");
    expect(message).toContain("accountId: monitor");
    expect(message).toContain("/Users/hd/Developer/cortana/tools/trading/run-trading-cron-alert.sh");
    expect(message).toContain("CANSLIM + Dip Buyer");
    expect(message).toContain("handles Telegram delivery directly");
